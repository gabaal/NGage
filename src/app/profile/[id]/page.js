...
export default async function ProfileID({params}) {
  ...
  const getProfile = await sql.query(`
    SELECT 
      ...
      users.bio, 
      users.location,   
      TO_CHAR(users.date_of_birth, 'DD Month') AS date_of_birth
    FROM 
      users
    WHERE 
      users.clerk_id = '${params.id}';
  `);
  ...
  const followersResult = await sql.query(`
    ...
    WHERE 
      follows.followed_clerk_id = '${params.id}';
  `);
  ...
  const followeesResult = await sql.query(`
    ...
    WHERE 
      follows.follower_clerk_id = '${params.id}';
  `);
